namespace StoreVouchers.Api.Filters
{
    ...
    using StoreVouchers.Api.Constants;
        ...
        public void OnActionExecuting(ActionExecutingContext context)
        {
            if (!context.ModelState.IsValid)
            {
                var response = new WebResponse
                {
                    Message = string.Join(
                      string.Empty, 
                      context.ModelState.Keys.SelectMany(key => context.ModelState[key].Errors.Select(x => new { key, x.ErrorMessage })).Select(a => a.ErrorMessage).ToList()),
